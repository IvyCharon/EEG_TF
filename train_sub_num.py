...
import random
import argparse
from data_process.process import data_process, get_eeg_dataloader_sub_n
...
def main(args):
    ...
    for p in range(fi.total_subjects_number[args.dataset]):
        src_loader, tgt_train_list, tgt_loader = get_eeg_dataloader_sub_n(p+1, data, label, args.dataset, args.batch_size, args.method, args.sub_n)
    ...
    output_path = os.path.join('output_sub_num', args.alg, args.dataset, str(args.sub_n), '{}.txt'.format(args.model, args.sub_n))
...
def train(src_loader, tgt_train_list, tgt_loader, tgt_num, args):
    save_model_path = os.path.join('output_sub_num', args.alg, args.dataset, str(args.sub_n))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sub_n', type=int, default=-1, help="subject n for train")

    args = parser.parse_args()
    setattr(args, "num_classes", fi.num_classes[args.dataset])
    setattr(args, "num_subject", fi.total_subjects_number[args.dataset] if args.sub_n == -1 else args.sub_n+1)
